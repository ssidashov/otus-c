...
// p - предикат - является ли число нечетным
...
//m - аналог map с функтором, возвращающим void, либо for_each
...
//f - аналог filter
struct list_element *filter(struct list_element *list,
                            struct list_element *result,
                            bool (*predicate)(ELEMENT_TYPE)) {
  if (list == NULL) {
    return result;
  }
...
int main(int argc, char **argv) {
  ...
  puts(empty_str);
  //нет освобождение памяти под списки.
  return 0;
}
